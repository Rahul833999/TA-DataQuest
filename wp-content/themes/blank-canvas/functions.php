...
// 		return wp_count_posts($args);
// 		return apply_filters( 'wp_count_posts', $counts, 'post', $perm );
	}

function get_ajax_posts() {
		$category = isset($_POST['cat'])?trim($_POST['cat']):"";
		$perpage = isset($_POST['perpage'])?trim($_POST['perpage']):"-1";
        // Query Arguments
        $args = array(
            'post_type' => array('post'),
            'post_status' => array('publish'),
        ...
        if(count($allposts) != 0):
            foreach($allposts as $key => $value):
                $extrapostdata = array("post" => $allposts[$key], "category" => array("cat_ID" => get_the_category($allposts[$key]->ID)[0]->cat_ID, "cat_name" => get_the_category($allposts[$key]->ID)[0]->name, "cat_slug" => get_the_category($allposts[$key]->ID)[0]->slug));
                array_push($postarray, $extrapostdata);
            endforeach;
            echo json_encode( array("status" => 200, "message" => "success", "data" => $postarray));
        else:
            echo json_encode( array("status" => 0, "message" => "fail", "data" => ""));
        endif;
        exit;
    }
...
function get_ajax_cats() {
	 	$category = isset($_POST['cat'])?trim($_POST['cat']):"";
        $allcats = get_categories();

        if(count($allcats) != 0):
            echo json_encode(array("status" => 200, "message" => "success", "data" => $allcats));
        else:
            echo json_encode( array("status" => 0, "message" => "fail", "data" => ""));
        endif;
        exit;
...
// Fire AJAX action for both logged in and non-logged in users
add_action('wp_ajax_get_ajax_cats', 'get_ajax_cats');
add_action('wp_ajax_nopriv_get_ajax_cats', 'get_ajax_cats');
